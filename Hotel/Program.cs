...
namespace Hotel
{
    class Program
    {
        ...
        private static bool SpecificRoom { get; set; }
		...
		private static void Booking(Hotel hotel, List<Tuple<string, int, int, string>> bookingsList, bool specific)
		{
            ...
            if (CheckIfNumber(startDay) && CheckIfNumber(endDay))
            {
				if (specific && CheckIfNumber(roomId))
				{
            ...
            if (answer == "y")
            {
				Booking(hotel, bookingsList, specific);
            }
            else
            {
        ...
        private static void TestCasesWithSpecificRoom()
        {
		...
		private static void TestCasesWithRandomRoom()
		{
            var casesList = new List<Tuple<string, int, int, string>>();
		...
		private static void PrintRoomsTable(List<Room> rooms)
		{
			...
			{
				t.AddRow($"Room {room.Id}", string.Join(", ", room.ReservedDays.OrderBy(x => x)));
			}
			t.Print();
		}
        ...
        private static void PrintToFile()
        {
			...
			TestCasesWithSpecificRoom();
            TestCasesWithRandomRoom();
